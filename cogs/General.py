...
class General(commands.Cog):
        ...
        async with aiosqlite.connect(self.bot.db) as db:
            ...
            for row in rows:
                resp = await manage_commands.add_slash_command(
                    self.bot.user.id, self.bot.http.token, row["guild_id"], row["name"], row["desc"], [self.option]
                )
                self.bot.slash.add_slash_command(
                    self.cmd_template, row["name"], row["desc"], [row["guild_id"]], [self.option]
                )

            await self.bot.slash.sync_all_commands(delete_from_unused_guilds=True)
    ...
    async def command_create(self, ctx, name, response, description=None):
        ...
        async with aiosqlite.connect(ctx.bot.db) as db:
            ...
            await manage_commands.add_slash_command(ctx.bot.user.id, ctx.bot.http.token,
                                                    ctx.guild_id, name, description, [self.option])

            await db.execute("INSERT INTO cmds VALUES (?, ?, ?, ?, ?);",
                             (name, response, description, ctx.author_id, ctx.guild_id))
            await db.commit()
    ...
    @slash.cog_subcommand(base="command", name="remove", description="Remove an existing custom command.",
                          guild_ids=config.guilds)
    async def command_remove(self, ctx, name=None):
        # TODO: Moderator or cmd owner
        if not ctx.author.guild_permissions.manage_messages:
            raise commands.MissingPermissions
        cmds = await manage_commands.get_all_commands(ctx.bot.user.id, ctx.bot.http.token, ctx.guild_id)
        cmds = [cmd['name'] for cmd in cmds]
        await ctx.send(str(cmds))
